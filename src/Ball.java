...
public class Ball extends PhysicsObject {
	...
	public void tick(int w, int h) {
		...
		super.applyForce(new Vector2D(0, 21*super.getMass()));
		
		for(PhysicsObject p: super.getHandler().getObjects()){
			if(p.getId() == ID.Ball){
				Ball b = (Ball) p;
				if(!b.getColor().equals(getColor())){
					if(Math.abs(getDistance(b)) < b.getRadius() + getRadius() - .1){
						System.out.println("Before"+getDistance(b));
						...
						ratio = getRadius()/(b.getRadius() + getRadius()) + .001;
						theta += Math.PI;
